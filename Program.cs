...
builder.Services.AddCors(policies =>
{
    ...
    {
        policy.AllowAnyHeader().WithOrigins("https://localhost:7290/").WithMethods(new string[] { "GET", "POST" });
    });
});

builder.Services.AddScoped(typeof(EmployeeRestAPI.Models.EF.EmployeeDbContext));
